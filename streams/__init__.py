...
``SinglyLinkedStream``. For the sake of brevity, you might want to
create an alias for the class (e.g. ``Stream = SinglyLinkedStream``).
...
class SinglyLinkedStream(LinearStream):
    ...
    def __init__(self, value, next_thunk, *, does_memoize=True):
        ...
        ``next_thunk`` is a zero-argument function that should return
        the following node.

        By default, the node will cache the result of ``next_thunk``.
        This can potentially hog a lot of memory. To turn caching off,
        set ``does_memoize`` to ``False``. It might be desirable to
        propagate this to composite streams generated by custom
        functions.

        Changing any of these values after initialization might cause
        unexpected results.
        """
    ...
    def filter(self, predicate=None):
        ...
        ``predicate`` is the function to apply to the values in the
    ...
    def map(cls, fn, *streams, does_memoize=True):
        ...
        ``fn`` is the function to be applied to each value in the
        stream.

        ``streams`` is the tuple of streams that contain the values to
        be mapped.
...
class DoublyLinkedStream(SinglyLinkedStream, Reversible):
    ...
    def __init__(
        ...
        ``previous_thunk`` is a zero-argument function that should
        return the preceding node. It defaults to behaving similarly to
        singly linked nodes.
    ...
    def _filter(self, predicate, traversal_direction):
        ...
        ``traversal_direction`` is the direction in which the nodes
        should be traversed.
        """
...
def thunk_init(thunk, init):
    ...
    ``thunk`` is the zero-parameter function that returns the object of
    interest.
