...
class LinearStream(Stream, Iterable, metaclass=ABCMeta):
    ...
    def __getitem__(self, key):
        ...
        ``key`` must be an integer or a slice object whose ``start``
        and ``stop`` values are integers. Negative indices are relative
        to ``self``.
        """
    ...
    def from_iterable(cls, iterable, does_memoize=True):
        ...
        Use of the iterable elsewhere afterward is generally
        inadvisable. Otherwise, the stream might become out of sync.
    ...
    def _from_iterator(cls, iterator, does_memoize=True):
        """Returns a new stream that contains data from an iterator.
        Use of the iterator elsewhere afterward is generally
        inadvisable. Otherwise, the stream might become out of sync.
